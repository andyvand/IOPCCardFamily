...
IOPCCardBridge::getNubResources(IOService * service)
{
	...
	err = nub->IOService::getResources();
    }
...
IOPCCardBridge::configureInterruptController(void)
{
    ...
    char * uniqueName = (char *)(kInterruptControllerNamePrefix "12345678");
    snprintf(uniqueName, strlen(uniqueName), "%s%08x", kInterruptControllerNamePrefix, uniqueNumber);
...
IOPCCardBridge::initializeDriverServices(void)
{
	...
	client_reg.event_callback_args.client_data = (void *)((uint64_t)i);
...
IOPCCardBridge::constructPCCard16Properties(IOPCCard16Device *nub)
{
	...
	if (cisinfo.Chains > 0) {
		...
		char nameStr[32];
		snprintf(nameStr, sizeof(nameStr), "pccard%x,%x", manfid.manf, manfid.card);
...
IOPCCardBridge::publishPCCard16Nub(IOPCCard16Device * nub, UInt32 socketIndex, UInt32 functionIndex)
{
	...
	char location[24];
	snprintf(location, sizeof(location), "%X,%X", socketIndex, functionIndex);
...
//	break;
    ...
    case CS_EVENT_EJECTION_COMPLETE:
	{
	    ...
	    IOPCCardEjectController * driver = OSDynamicCast(IOPCCardEjectController,
							    provider->copyClientWithCategory(matchString));
	    matchString->release();
	    if (!driver) break;
...
IOPCCardBridge::createCardBusNub(UInt8 socket, UInt8 function)
{
	...
	spaceFromProperties(propTable, &((class IOCardBusDevice *)nub)->space);
...
#if defined(__i386__) || defined(__x86_64__)
	UInt32 base0 = bridgeDevice->configRead32(kIOPCIConfigBaseAddress0);
	IOMemoryDescriptor* memDesc0 = IOMemoryDescriptor::withPhysicalAddress((IOPhysicalAddress) base0,(IOByteCount) 0xff,kIODirectionOutIn);
	if (memDesc0 != NULL) {
		cardBusRegisterMap = memDesc0->map();
		cardBusRegisterMap->retain();
	}
#else
