...
#include "../pccard/config.h"
#include "../pccard/k_compat.h"
...
static int __init get_pci_irq(socket_info_t *s)
{
    u16 irq;
#ifdef __MACOSX__
    pci_readb(s, PCI_INTERRUPT_LINE, (u8)&irq);
#endif
#ifdef __BEOS__
...
static u_int __init cirrus_set_opts(socket_info_t *s, char *buf)
{
	...
	strncat(buf, 128, " [ring]");
    if (p->misc2 & PD67_MC2_DYNAMIC_MODE)
	strncat(buf, 128, " [dyn mode]");
    if (p->misc1 & PD67_MC1_INPACK_ENA)
	strncat(buf, 128, " [inpack]");
	...
	if ((s->flags & IS_PCI) && (irq_mode == 1) && get_pci_irq(s)) {
	    ...
	    buf += strlen(buf);
        snprintf(buf, 128, " [pci irq %d]", s->cap.pci_irq);
	    mask = 0;
	}
#endif
    }
	...
	buf += strlen(buf);
	snprintf(buf, 128, " [%d/%d/%d] [%d/%d/%d]", p->timer[0], p->timer[1],
...
static u_int __init ti113x_set_opts(socket_info_t *s, char *buf)
{
	...
	if (p->sysctl & TI113X_SCR_CLKRUN_SEL) {
	    strncat(buf, 128, " [clkrun irq 12]");
	    mask &= ~0x1000;
	} else {
	    strncat(buf, 128, " [clkrun irq 10]");
	    mask &= ~0x0400;
	}
    }
    switch (p->devctl & TI113X_DCR_IMODE_MASK) {
    case TI12XX_DCR_IMODE_PCI_ONLY:
	strncat(buf, 128, " [pci only]");
	mask = 0;
	break;
    case TI113X_DCR_IMODE_ISA:
	strncat(buf, 128, " [isa irq]");
	if (old) mask &= ~0x0018;
	break;
    case TI113X_DCR_IMODE_SERIAL:
	strncat(buf, 128, " [pci + serial irq]");
	mask = 0xffff;
	break;
    case TI12XX_DCR_IMODE_ALL_SERIAL:
	strncat(buf, 128, " [serial pci & irq]");
	mask = 0xffff;
	break;
    }
    return mask;
...
static u_int __init ricoh_set_opts(socket_info_t *s, char *buf)
{
	...
	if (p->misc & RL5C47X_MISC_SRIRQ_ENA)
        snprintf(buf, 128, " [serial irq]");
	else
	    snprintf(buf, 128, " [isa irq]");
	buf += strlen(buf);
    }
    snprintf(buf, 128, " [io %d/%d/%d] [mem %d/%d/%d]",
...
static u_int __init o2micro_set_opts(socket_info_t *s, char *buf)
{
    ...
    if (p->mode_d & O2_MODE_D_ISA_IRQ) {
        strncat(buf, 128, " [pci+isa]");
    } else {
        switch (p->mode_d & O2_MODE_D_IRQ_MODE) {
            case O2_MODE_D_IRQ_PCPCI:
                strncat(buf, 128, " [pc/pci]"); break;
            case O2_MODE_D_IRQ_PCIWAY:
                strncat(buf, 128, " [pci/way]"); break;
            case O2_MODE_D_IRQ_PCI:
                strncat(buf, 128, " [pci only]"); mask = 0; break;
        }
    }
    if (s->flags & IS_CARDBUS) {
	if (p->mode_d & O2_MODE_D_W97_IRQ)
        strncat(buf, 128, " [win97]");
    }
    return mask;
}
...
static u_int __init topic_set_opts(socket_info_t *s, char *buf)
{
    ...
    snprintf(buf, 128, " [slot 0x%02x] [ccr 0x%02x] [cdr 0x%02x] [rcr 0x%02x]",
...
static void __init cb_set_opts(socket_info_t *s, char *buf)
{
    ...
    if (s->cap.pci_irq == 0)
	strncat(buf, 128, " [no pci irq]");
    else
	snprintf(buf, 128, " [pci irq %d]", s->cap.pci_irq);
    buf += strlen(buf);
    if (!(s->flags & IS_TOPIC))
	s->cap.features |= SS_CAP_PAGE_REGS;
    snprintf(buf, 128, " [lat %d/%d] [bus %d/%d]",
